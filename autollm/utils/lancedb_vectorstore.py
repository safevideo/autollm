...
class LanceDBVectorStore(VectorStore):
    """
    Updated LanceDB Vector Store to support both local and cloud-based storage.

    Args:
        uri (str): Location for LanceDB storage or cloud database path.
        table_name (str, optional): Table name for embeddings storage. Defaults to "vectors".
        nprobes (int, optional): Number of probes for search accuracy. Defaults to 20.
        refine_factor (int, optional): Refine factor for re-ranking results. Defaults to None.
        api_key (str, optional): API key for cloud-based connection. Defaults to None.
        region (str, optional): Region for cloud-based connection. Defaults to None.
        **kwargs (Any): Additional keyword arguments.

    Raises:
        ImportError: If `lancedb` package is not found.
    ...
    def query(
        ...
        **kwargs: Any,
        ...
        table = self.connection.open_table(self.table_name)
        lance_query = (
            table.search(query.query_embedding).limit(query.similarity_top_k).where(where).nprobes(
                self.nprobes))
